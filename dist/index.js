...
if (canvas instanceof HTMLCanvasElement) {
    ...
    canvas.addEventListener("pointerleave", () => {
        if (state.mode !== "ball" && !motionTrackingEnabled)
            applyTiltInput(0);
    });
    ...
    const disturbWater = (normalizedX, strength) => {
        if (waterSurface.count === 0)
            return;
    ...
    const updateWater = (dt) => {
        ...
        for (let step = 0; step < substeps; step += 1) {
            ...
            for (let iteration = 0; iteration < 2; iteration += 1) {
                ...
                for (let i = 0; i < waterSurface.count; i += 1) {
                    if (i > 0)
                        waterSurface.heights[i - 1] += waterSurface.leftDeltas[i];
                    if (i < waterSurface.count - 1)
                        waterSurface.heights[i + 1] += waterSurface.rightDeltas[i];
                }
            }
            for (let i = 0; i < waterSurface.count; i += 1) {
                waterSurface.heights[i] += waterSurface.velocities[i] * stepDt;
                waterSurface.heights[i] = clamp(waterSurface.heights[i], tank.y + tank.height * 0.14, tank.y + tank.height * 0.94);
            }
        }
    ...
    const getWaterSurfaceY = (x) => {
        if (waterSurface.count === 0)
            return tank.y + tank.height * 0.58;
        ...
        return clamp(interpolatedHeight + microRipple + shimmer, tank.y + tank.height * 0.16, tank.y + tank.height * 0.92);
    };
    const drawWater = () => {
        ctx.clearRect(0, 0, state.width, state.height);
        const radius = 28;
