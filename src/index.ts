...
const objectButtons = Array.from(document.querySelectorAll<HTMLButtonElement>("[data-object]"));
...
if (!(browserClose instanceof HTMLButtonElement)) throw new Error("Close button was not found.");
...
const resetCurrentMode = () => {
  ...
  if (state.mode === "water") resetWater();
};
...
const resizeCanvas = () => {
  ...
  if (items.length === 0) resetBallScene();
};
...
canvas.addEventListener("pointerleave", () => {
  if (state.mode === "water") tilt.target = 0;
});
