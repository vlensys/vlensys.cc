...
(() => {
    ...
    const POOL_SIZE = 5;
    const host = document.querySelector('.column-bg');
    if (!host || !BACKGROUNDS.length)
        return;
    const empty = () => ({ windowStart: 0, pool: [], seen: [], last: '' });
    const known = (list) => Array.isArray(list)
        ? list.filter((v) => typeof v === 'string' && BACKGROUNDS.indexOf(v) !== -1)
        : [];
    function read() {
        try {
